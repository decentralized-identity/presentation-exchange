...
var TabPanels = globalThis.TabPanels = class TabPanels extends HTMLElement {
  ...
  connectedCallback(){
    let target = this.querySelector(`nav [href="${location.hash}"]`);
    if (target && target.parentNode?.parentNode === this) {
      this.setAttribute('selected-index', Array.from(target.parentNode.children).indexOf(target))
    }
  }
