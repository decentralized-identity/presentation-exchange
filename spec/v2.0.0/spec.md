...
**Ratification Date**: November 3rd, 2022
  ...
  1. If the `constraints` property of the [[ref:Input Descriptor]] is present,
     ...
     1. For each [JSONPath](https://goessner.net/articles/JsonPath/) expression
        ...
        1. If the result returned no JSONPath match, skip to the next
          `path` array element.
        2. Else, evaluate each JSONPath match (_candidate_) as follows until one is found 
           that satisfies the _fields object_ constraints:
...
```json
[[insert: ./test/v2.0.0/presentation-submission/appendix_JWT_example.json]]
```
:::
